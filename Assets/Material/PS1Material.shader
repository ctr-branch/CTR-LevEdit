        ...
        fixed4 frag (v2f i, fixed facing : VFACE) : SV_Target { 
            ...
            if(i.uv.y>1.0) {
                if(i.uv.x<1.0) {
                    ...
                    uv.y *= -1.0;
                        col = ( facing > 0 ? i.color : i.color ) * tex2D(_Tex2, uv); 
                } else {
                    uv = flagrot( uv, _flipRotate3);
                    uv.y *= -1.0;
                        col =  ( facing > 0 ? i.color : i.color ) * tex2D(_Tex3, uv); 
                }
            } else { 
                if(i.uv.x<1.0) {
                    ...
                    //uv.x *= -1.0; //midunk =18 and not 17
                    uv.y *= -1.0;
                        col =  ( facing > 0 ? i.color : i.color ) * tex2D(_Tex0, uv); 
                } else {
                    uv = flagrot(uv, _flipRotate1);
                    uv.y *= -1.0;
                    //uv.y *= -1.0; //midunk =18 and not 17
                        col =  ( facing > 0 ? i.color : i.color ) * tex2D(_Tex1, uv); 
                }
            }   
